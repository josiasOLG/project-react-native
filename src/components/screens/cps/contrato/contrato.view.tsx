...
const ContratosView: React.FC<ContratosViewProps> = ({
  item,
  tela,
  TelaEnum,
  formatarReal,
      ...
      {item.product.type === 1 ? (
        <View style={[styles.flex]}>
          ...
          <View style={[styles.col2, styles.centerContent]}>
            <TouchableOpacity
              onPress={() => downloadFile(item.ccbUri)}
              style={styles.centerContent}
            >
              <Image
                source={require('../../../../assets/images/baixar.png')}
                resizeMode="contain"
                style={styles.image}
              />
              <Text style={styles.textoBaixar}>download ccb</Text>
            </TouchableOpacity>
          </View>
        </View>
      ) : (
